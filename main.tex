...
\begin{document}
\title*{Error Propagation for Functions of Multiple Expectations in Monte Carlo and Quasi-Monte Carlo}
...
Moreover, $\,\boldsymbol{D}$ may be used to compute individual uncertainty levels $\boldsymbol{\alpha}^{(\boldsymbol{\mu})}$ by adapting \eqref{SoRa_eq:alpha_mu_from_alpha_s} to the number of individual solutions contributing to each combined solution. The idea is to ensures that each combined uncertainty is greater than the sum of all its dependent individual uncertainties. Specifically, let $\boldsymbol{N} \in \mathbb{N}^{\boldsymbol{d}_{\boldsymbol{s}}}$ be the number of dependent individual solutions for each combined solution. For any $\boldsymbol{1} \leq \boldsymbol{l} \leq \boldsymbol{d}_{\boldsymbol{s}}$ , $N_{\boldsymbol{l}}$ can be determined by counting the number of $\text{True}$ values resulting from evaluating $\boldsymbol{D}$ at the multi-dimensional array with only the $\boldsymbol{l}^\text{th}$ entry set to $\text{True}$. For any $\boldsymbol{1} \leq \boldsymbol{k} \leq \boldsymbol{d}_{\boldsymbol{\mu}}$ we set 
\begin{equation}
    \alpha_{\boldsymbol{k}}^{(\boldsymbol{\mu})} = \frac{\alpha_{\boldsymbol{l}_{\boldsymbol{k}}}^{(\boldsymbol{s})}}{N_{\boldsymbol{l}_{\boldsymbol{k}}}}
    \label{SoRa_eq:alpha_mu_from_alpha_s_vec}
\end{equation}
where 
${\boldsymbol{l}_{\boldsymbol{k}}}$ is the multi-index corresponding to the maximum $\boldsymbol{N}$ among all multi-indices for which the $\boldsymbol{k}^\text{th}$ individual solution is a dependency. 
